...
public class RecommendationRule4 implements RecommendationRule {
    private final UserService userService;

    public RecommendationRule4(UserService userService) {
    ...
    public Restaurant[] applyRecommendationRule(User user, Restaurant[] availableRestaurants) {
        ...
        for(Restaurant restaurant : availableRestaurants){
            if(secondaryCuisine.contains(restaurant.getCuisine()) && restaurant.getCostBracket()==primaryCostBracket && restaurant.getRating()>=4){
